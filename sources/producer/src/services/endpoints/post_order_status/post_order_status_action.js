...
class Action {
        ...
        sqlite.getOrders( "uuid", uuid, ( err, orders ) => {
            ...
            if ( shouldSpawnChildren ) {
                ...
                this.performUpdate( uuid, updates, ( err, status ) => {
                    if ( parentUuid ) this.checkParentCompletion( order, parentUuid, () => {} );
                    callback( err, status );
                } );
            }
        } );
    ...
    checkParentCompletion( order, parentUuid, callback ) {
        const { status } = this.parameters;
